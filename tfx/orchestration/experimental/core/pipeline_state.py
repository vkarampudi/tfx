...
class PipelineState:
  ...
  def new(
    ...
    # Dual-writes pipeline run to both Tflex and MLMD db.
    ...
    if pipeline_asset:
      try:
        ...
        logging.exception('Failed to create pipeline run.')
    ...
    def _pre_commit(original_execution, modified_execution):
      ...
      try:
        env.get_env().update_pipeline_run_status(
            ...
            original_execution,
            modified_execution,
            _get_sub_pipeline_ids_from_pipeline_info(
                self.pipeline.pipeline_info
            ),
        )
      except Exception:  # pylint: disable=broad-except
        logging.exception('Failed to update pipeline run status.')
